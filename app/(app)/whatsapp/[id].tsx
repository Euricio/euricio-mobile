...
export default function WhatsAppChatScreen() {
  ...
  const { data: messages, isLoading } = useWhatsAppMessages(id);
  const sendMessage = useSendWhatsAppMessage();
  const assignConversation = useAssignWhatsAppConversation();

  const contactName = conversation?.contact_name || conversation?.contact_number || '—';
  ...
  const handleSend = useCallback(() => {
    ...
    sendMessage.mutate(
      { conversationId: id, body: message.trim() },
      {
        ...
        onError: () => Alert.alert(t('error'), t('whatsapp_sendError')),
      },
    );
  ...
  const handleAssign = useCallback(() => {
    if (!id) return;
    Alert.alert(t('whatsapp_assignAgent'), '', [
      { text: t('cancel'), style: 'cancel' },
    ]);
  }, [id, t]);

  const renderMessage = useCallback(
    ({ item }: { item: WhatsAppMessage }) => {
    ...
    >
      <Stack.Screen
        options={{
          ...
          headerRight: () => (
            <TouchableOpacity onPress={handleAssign} style={styles.headerBtn}>
              <Ionicons
      ...
      {conversation?.entity_type && (
        <View style={styles.linkedBanner}>
          ...
          <Text style={styles.linkedText}>
            {t('whatsapp_linked')}: {conversation.entity_type}
          </Text>
        </View>
      )}
      ...
      <View style={styles.inputBar}>
        <TextInput
          ...
          placeholder={t('whatsapp_messageHint')}
